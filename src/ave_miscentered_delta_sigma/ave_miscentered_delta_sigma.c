...
int calc_ave_miscentered_delta_sigma(double*R,int NR,
  ...
  //#pragma omp parallel for shared(R,Rbins,miscentered_delta_sigma,ave_miscentered_delta_sigma) private(i,lRlow,lRhigh,Rlow,Rhigh) //This is fast enough to not need parallelization
  for( i = 0; i < Nbins; i++){
    lRlow = lRmin+i*dlR;
