...
int do_integral(double*mis_sigma,double*err,integrand_params*params){
  ...
  status = gsl_integration_qag(&F,lrmin-10,lrmax,
...
double integrand_outer(double lRc,void*params){
  ...
  }else if((Rp2plusRc2 - RpRctimes2) > rmin*rmin && 
	   (Rp2plusRc2 + RpRctimes2) < rmax*rmax){
  ...
  status |= gsl_integration_qag(&F,0,PI,TOL2,TOL2/10.,
				workspace_size,6,workspace,&result,&abserr);
  return Rc*P_mis(Rc,pars->Rmis_sq)*result;
  //Factor of 2 cancels the denominator. PI is factored out
}

//This is the function that needs to be broken into three parts
//Basically this will amount to putting the if statements 
//in the outer integrand.
double integrand_inner_boundaries(double theta,void*params){
